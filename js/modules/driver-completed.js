...
function driverDisplayName(driverId) {
    const value = String(driverId || "Driver");
    return value.replace(/driver\s*/i, "Driver ").trim();
}
    ...
    (snapshot) => {
        ...
        snapshot.forEach((documentSnapshot) => {
            ...
            const belongsToCurrentDriver =
                String(trip.assignedDriver || "").trim() ===
                String(CURRENT_DRIVER_ID).trim();

            const isCompleted =
                String(trip.status || trip.driverStatus || "")
                    .trim()
...
function createCompletedTripCard(trip) {
    ...
    const driverName =
        ...
        driverDisplayName(CURRENT_DRIVER_ID);
